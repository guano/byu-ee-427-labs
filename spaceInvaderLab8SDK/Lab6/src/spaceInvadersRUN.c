...
//	xil_printf("switches forever\n\r");
	...
	if(currentSWITCHESState & SWITCH_6){
		...
		XTmrCtr_Initialize(&happy_timer, XPAR_AXI_TIMER_0_DEVICE_ID);
		XTmrCtr_Start(&happy_timer, 0);
		int i;
		for(i=0;i<FRAME_BUFFER_SIZE;i++){			// Go through whol
			framePointer1[i] = framePointer0[i];
		}
		XTmrCtr_Stop(&happy_timer, 0);
		xil_printf("Software copy timer reads: %d\n\r", XTmrCtr_GetValue(&happy_timer, 0));
	}
	...
	if(currentSWITCHESState & SWITCH_7){
		...
		cleanup_platform();
		XTmrCtr_Initialize(&happy_timer, XPAR_AXI_TIMER_0_DEVICE_ID);
		xil_printf("happy timer initialized\n\r");
		XTmrCtr_Start(&happy_timer, 0);
...
void dma_interrupt_handler() {
	...
	XTmrCtr_Stop(&happy_timer, 0);
	xil_printf("Hardware copy timer reads: %d\n\r", XTmrCtr_GetValue(&happy_timer, 0));
}
