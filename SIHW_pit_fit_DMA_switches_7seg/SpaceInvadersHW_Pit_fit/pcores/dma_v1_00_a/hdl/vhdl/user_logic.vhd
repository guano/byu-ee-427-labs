...
-- Date:              Wed Dec 07 14:57:15 2016 (by Create and Import Peripheral Wizard)
...
architecture IMP of user_logic is
  ...
  signal slv_reg_write_sel              : std_logic_vector(9 downto 0);
  signal slv_reg_read_sel               : std_logic_vector(9 downto 0);
  ...
  type CMD_CNTL_SM_TYPE is (CMD_IDLE, CMD_READ, CMD_WAIT_FOR_READ_DATA, CMD_WRITE, CMD_WAIT_FOR_WRITE_DATA, CMD_DONE);
...
begin
  ...
  -- 
  ------------------------------------------
  slv_reg_write_sel <= Bus2IP_WrCE(9 downto 0);
  slv_reg_read_sel  <= Bus2IP_RdCE(9 downto 0);
  ...
  -- 
  ------------------------------------------
  mst_reg_write_req <= Bus2IP_WrCE(10) or Bus2IP_WrCE(11) or Bus2IP_WrCE(12) or Bus2IP_WrCE(13);
  mst_reg_read_req  <= Bus2IP_RdCE(10) or Bus2IP_RdCE(11) or Bus2IP_RdCE(12) or Bus2IP_RdCE(13);
  mst_reg_write_sel <= Bus2IP_WrCE(13 downto 10);
  mst_reg_read_sel  <= Bus2IP_RdCE(13 downto 10);
  ...
  IP2Bus_Mst_Reset  <= mst_cmd_sm_reset;
	...
	--USER LOGIC--------------------------------------------
	--output is the interupt 
	DMA_COMPLETE_INTERRUPT <= interrupt;
	--------------------------------------------------------
    ...
    if ( Bus2IP_Clk'event and Bus2IP_Clk = '1' ) then
      if ( Bus2IP_Resetn = '0' ) then
        ...
        case mst_cmd_sm_state is
				...
				when CMD_IDLE =>--Starting State
            if (mst_go = '1' ) then
              mst_cmd_sm_state  <= CMD_READ;--Go to read 
              mst_cmd_sm_clr_go <= '1';--Reset mst_go to 0 
            else
              mst_cmd_sm_state  <= CMD_IDLE;--Stay in Idle
              mst_cmd_sm_busy   <= '0';--busy signal
            end if;
					--READ--------------------------------------------------------------------------------------------------
          when CMD_READ =>--Read state
            if ( Bus2IP_Mst_CmdAck = '1' and Bus2IP_Mst_Cmplt = '0' ) then--Acknoledge state but not compleated  
              mst_cmd_sm_state <= CMD_WAIT_FOR_READ_DATA;-- go to Wait state to read data
            else
              mst_cmd_sm_state       <= CMD_READ;--Stay in state 
              mst_cmd_sm_rd_req      <= '1';--read request 
              mst_cmd_sm_wr_req      <= '0';--write request 
              mst_cmd_sm_ip2bus_addr <= slv_reg0 + currentIndex;--index address 
              mst_cmd_sm_ip2bus_be   <= mst_ip2bus_be(15 downto 16-C_MST_DWIDTH/8 );
              mst_cmd_sm_bus_lock    <= mst_cntl_bus_lock;
            end if;

          when CMD_WAIT_FOR_READ_DATA =>--wait for read data 
            if ( Bus2IP_Mst_Cmplt = '1' ) then--Compleated signal 
              mst_cmd_sm_state <= CMD_WRITE;--Go to write state
              if (Bus2IP_Mst_Cmd_Timeout = '1') then--Error checking
                -- AXI4LITE address phase timeout
                mst_cmd_sm_set_error   <= '1';--Send error report 
                mst_cmd_sm_set_timeout <= '1';--Send error report 
              elsif ( Bus2IP_Mst_Error = '1' ) then
                -- AXI4LITE data transfer error--Errror checking
                mst_cmd_sm_set_error   <= '1';--Send error report 
              end if;
            else
              mst_cmd_sm_state <= CMD_WAIT_FOR_READ_DATA;--stay in state until compleated signal 
            end if;
						
					--WRITE-----------------------------------------------------------------------------------------------------
					when CMD_WRITE =>--write state 
            if ( Bus2IP_Mst_CmdAck = '1' and Bus2IP_Mst_Cmplt = '0' ) then--Acknoledge state but not compleated
              mst_cmd_sm_state <= CMD_WAIT_FOR_WRITE_DATA;--go to waite for write data 
            else
              mst_cmd_sm_state       <= CMD_WRITE;--Stay in state 
              mst_cmd_sm_rd_req      <= '0';--Read request 
              mst_cmd_sm_wr_req      <= '1';--Write request 
              mst_cmd_sm_ip2bus_addr <= slv_reg1 + currentIndex;--increment address 
              mst_cmd_sm_ip2bus_be   <= mst_ip2bus_be(15 downto 16-C_MST_DWIDTH/8 );
              mst_cmd_sm_bus_lock    <= mst_cntl_bus_lock;
            end if;

          when CMD_WAIT_FOR_WRITE_DATA =>--waite for data state 
            if ( Bus2IP_Mst_Cmplt = '1' ) then-- compleate signals 
              mst_cmd_sm_state <= CMD_DONE;-- go to done state 
              if ( Bus2IP_Mst_Cmd_Timeout = '1' ) then--checking for errors 
                -- AXI4LITE address phase timeout
                mst_cmd_sm_set_error   <= '1';--send error report 
                mst_cmd_sm_set_timeout <= '1';--send error report 
              elsif ( Bus2IP_Mst_Error = '1' ) then--check for error 
                -- AXI4LITE data transfer error
                mst_cmd_sm_set_error   <= '1';--send error
              end if;
            else
              mst_cmd_sm_state <= CMD_WAIT_FOR_WRITE_DATA;--Wait in state until compleate signal 
            end if;

          when CMD_DONE =>--Done state 
						if(currentIndex < slv_reg2) then--If slave_reg2/length is larger than index
							mst_cmd_sm_state  <= CMD_READ;-- go to read state 
							currentIndex <= currentIndex + 4;--increment index 
						else--finished 
							currentIndex <= (others => '0');--reset the index 
							mst_cmd_sm_state    <= CMD_IDLE;--go to idle state 
							mst_cmd_sm_set_done <= '1';--Done signal 
							mst_cmd_sm_busy     <= '0';--busy signal  
							interrupt <= '1'; -- fire interupt when finished 
						end if;
  ...
  DATA_CAPTURE_FIFO_I : entity proc_common_v3_00_a.srl_fifo_f
    ...
    (
      ...
      Data_Out   => IP2Bus_MstWr_d,
			--USER LOGIC----------------------------------
			--replaced open with port map to wire 
      FIFO_Full  => FIFO_Full,
  ...
  ------------------------------------------
	...
	--Put on data buss either slave or master or all zeros 
